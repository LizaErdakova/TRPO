  ...
  try {
    ...
    if (req.user) {
      ...
      sqlQuery = `
        ...
        LIMIT ?
      `;
      params = [searchPattern, req.user.userId, limit];
    } else {
...
router.post('/add', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (safeProductData.isPublic) {
      ...
      checkParams = [safeProductData.name, req.user.userId];
    }
    ...
    const insertParams = [
      ...
      safeProductData.isPublic ? null : req.user.userId,
...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    const [countResult] = await pool.query(countSql, req.user ? [req.user.userId] : []);
